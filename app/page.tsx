  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      ...
      try {
        ...
        // 1. Update info (Only if changed)
        setDeviceInfo(prev => {
          if (prev.name === data.deviceName && prev.room === data.roomCode) return prev;
          return { name: data.deviceName, room: data.roomCode };
        });

        // 2. Update activity (Only if ID or critical data changed)
        setActivity(prev => {
          const newData = data.activity || null;
          if (!prev && !newData) return null;
          if (prev?.id === newData?.id && prev?.title === newData?.title) return prev;
          return newData;
        });

        // 3. Update ad (Only if ID changed)
        setAd(prev => {
          const newData = data.ad || null;
          if (!prev && !newData) return null;
          if (prev?.id === newData?.id) return prev;
          return newData;
        });
        ...
        <main className="flex-1 flex items-stretch justify-center p-6 lg:p-4 gap-8 overflow-hidden relative">
          ...
          {activity?.qrCode ? (
            ...
            <>
              ...
              {/* Right Panel: Fixed QR Code (Only shown for Standard Activities) */}
              {activity && !activity.qrCode && (
                <div className="w-[30%] max-w-sm shrink-0 bg-white rounded-3xl shadow-2xl flex flex-col items-center justify-center p-8 text-slate-900 animate-in zoom-in-50 duration-500 h-full max-h-full relative overflow-hidden">
                  ...
                  <p className="text-center text-slate-500 mt-2 shrink-0 text-sm">Register your attendance with your phone.</p>
